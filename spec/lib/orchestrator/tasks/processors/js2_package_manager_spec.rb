...
RSpec.describe Orchestrator::Tasks::Processors::JS2PackageManager do
  let(:subject) { described_class.new date, quintes_info, courses_info, js2 }
  let(:quintes_info) { Orchestrator::Tasks::Extractors::PronosticFacile::JS2Package.new(date) }
  let(:courses_info) { Orchestrator::Tasks::Extractors::PronosticFacile::RacesInfo.new(date) }
  let(:js2)          { Orchestrator::Tasks::Extractors::Publisher::JS2Package.new(date) }
  let(:date) { '2016-08-15' }
  let(:expected_date) { "15 août 2016" }
  let(:race_id) { 143761 }
 ...
 ### EXPECTED DATA
  ...
  context '#json' do
    ...
    it 'contains information about date' do
      expect(subject.json[:date]).to eq(expected_date)
    end

    it 'contains information about pronostic' do
      expect(subject.json[:pronostic]).to eq(expected_transformed_json[:pronostic])
    end

    it 'contains race_comment' do
      expect(subject.json[:race_comment]).to eq(expected_transformed_json[:race_comment])
    end

    context 'contains information about partants where' do
      it 'contains 18 hourses' do
        expect(subject.json[:partants].count).to eq(expected_transformed_json[:partants].count)
      end

      it 'each horse has numero' do
        expect(subject.json[:partants].map{|h| h[:numero]}.sort).to eq(expected_transformed_json[:partants].map{|h| h[:numero]}.sort)
      end

      it 'each horse has nom' do
        expect(subject.json[:partants].map{|h| h[:nom]}.sort).to eq(expected_transformed_json[:partants].map{|h| h[:nom]}.sort)
      end

      it 'each horse has comment' do
        expect(subject.json[:partants].map{|h| h[:comment]}.sort).to eq(expected_transformed_json[:partants].map{|h| h[:comment]}.sort)
      end

      it 'each horse has note' do
        expect(subject.json[:partants].map{|h| h[:note]}.sort).to eq(expected_transformed_json[:partants].map{|h| h[:note]}.sort)
      end

      it 'each horse has group' do
        expect(subject.json[:partants].map{|h| h[:group]}.sort).to eq(expected_transformed_json[:partants].map{|h| h[:group]}.sort)
      end

    end

    it 'returns race_title' do
      expect(subject.json[:race_title]).to eq(expected_transformed_json[:race_title])
    end

    it 'returns cdj' do
      expect(subject.json[:cdj]).to eq(expected_transformed_json[:cdj])
    end

    it 'returns js' do
      expect(subject.json[:js]).to eq(expected_transformed_json[:js])
    end

    it 'returns expected json' do
      expect(subject.json).to eq(expected_transformed_json)
    end
  end
